...
func NormalizeInboundEndpoint(path string) string {
	...
	case strings.Contains(path, "/v1/videos/") || strings.Contains(path, "/videos/"):
...
func DeriveUpstreamEndpoint(inbound, rawRequestPath, platform string) string {
	...
	case service.PlatformOpenAI:
		if inbound == EndpointEmbeddings || inbound == EndpointImagesGenerations || inbound == EndpointImagesEdits || inbound == EndpointVideosGenerations || inbound == EndpointVideosStatus {
			return inbound
		}
