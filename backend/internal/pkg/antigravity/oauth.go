...
const (
	...
	// Antigravity OAuth 客户端凭证（环境变量名）
	// Deployers should set ANTIGRAVITY_OAUTH_CLIENT_ID with the value that was previously hardcoded.
...
// ClientID is the Antigravity OAuth client ID. Populated from env var or base64-encoded
// fallback (to avoid GitHub secret scanning false positives).
var ClientID string
...
func init() {
	// 从环境变量读取 client_id，未设置则使用 base64 编码的默认值
	if id := os.Getenv(AntigravityOAuthClientIDEnv); id != "" {
		ClientID = id
	} else {
		b, _ := base64.StdEncoding.DecodeString("MTA3MTAwNjA2MDU5MS10bWhzc2luMmgyMWxjcmUyMzV2dG9sb2poNGc0MDNlcC5hcHBzLmdvb2dsZXVzZXJjb250ZW50LmNvbQ==")
		ClientID = string(b)
	}
